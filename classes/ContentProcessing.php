        ...
        foreach ($contentImages[0] as $imgHTML) {
            if (!preg_match($skipImagesRegex, $imgHTML)) {
                ...
                if (!empty($imgSrcSet)) {
                    ...
                    $replaceHTML = preg_replace( '/<img(.*?)srcset\s*=\s*[\'"]([^\'"]*)[\'"](.*?)/is', '<img$1srcset="' . $this->emptyImg . '"$3', $imgHTML);
                } else {
                    $imgSrcSet = $imgSrc;
                    $replaceHTML = preg_replace( '/<img/is', '<img srcset="' . $this->emptyImg . '"', $imgHTML);
                }
    ...
    private function addLazyLoadVideoIFrameContent()
    {
        ...
        foreach ($matches[0] as $iframeHTML) {
            if (!preg_match($skipFramesRegex, $iframeHTML)) {
                ...
                $replaceIframeHtml = sprintf(
                '<input type="text" aria-label="%1$s" class="l-d-none %1$s" data-original_content="%2$s">',
                    self::LOZAD_BASE_CLASS,
                    base64_encode($iframeHTML)
                );
    ...
    private function addLazyLoadVideoContent()
    {
        ...
        foreach ($videoWithPosterTags[0] as $videoWithPosterTag) {
            ...
            foreach ($sourceTagMatches[0] as $sourceTagMatch) {
                $sourceTagReplace = preg_replace('/<(.*)src=/is', '<$1 src="' . $this->emptyImg . '" data-src=', $sourceTagMatch);
